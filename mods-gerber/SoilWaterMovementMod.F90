...
!#6
  ...
  subroutine soilwater_zengdecker2009(bounds, num_hydrologyc, filter_hydrologyc, &
    ...
    associate(& 
         ...
         qflx_lat_aq       =>    waterfluxbulk_inst%qflx_lat_aq_col       &   ! Ootup:  [real(r8) (:)   ]  lateral transfer of water from hummock to hollow (mm/s)
         )
     ...
     !compute lateral flux in aquiferz
         if (jwt(c) .lt. nlevsoi) then
            ! sg: the original code has a loop through all soil levels, but I think that is not needed. 
