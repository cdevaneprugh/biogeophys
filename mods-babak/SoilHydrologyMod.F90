...
! MUST agree with name in namelist and read statement
  ...
  subroutine SetFloodc(num_nolakec, filter_nolakec, col, &
   ...
   subroutine SetQflxInputs(bounds, num_hydrologyc, filter_hydrologyc, &
     ...
     associate( &
          ...
          qflx_surf_input => waterdiagnosticbulk_inst%qflx_surf_input
     ...
     do fc = 1, num_hydrologyc
        ...
        if (snl(c) >= 0) then
          ...
          qflx_in_soil(c) = (1._r8 - frac_h2osfc(c)) * (qflx_top_soil(c) - qflx_sat_excess_surf(c) + qflx_surf_input(c))
          qflx_in_h2osfc(c) = frac_h2osfc(c) * (qflx_top_soil(c) - qflx_sat_excess_surf(c) + qflx_surf_input(c))
        #else 
   ...
   subroutine Infiltration(bounds, num_hydrologyc, filter_hydrologyc, &
     ...
     associate( &
          ...
          qflx_lat_aqu   => waterstatebulk_inst%qflx_lat_aqu
...
#if (defined HUM_HOL)
     !compute lateral flux in aquiferz
  ...
  do j = 1,nlevsoi
   ...
   subroutine Drainage(bounds, num_hydrologyc, filter_hydrologyc, num_urbanc, filter_urbanc,  &
        temperature_inst, soilhydrology_inst, soilstate_inst, waterstatebulk_inst, waterfluxbulk_inst, icefrac)
     !
     ! !DESCRIPTION:
     ...
     integer                  , intent(in)    :: filter_urbanc(:)     ! column filter for urban points
     integer                  , intent(in)    :: filter_hydrologyc(:) ! column filter for soil points
     real(r8), intent(in) :: icefrac(bounds%begc:bounds%endc,1:nlevsoi)   ! fraction of ice in layer
     type(temperature_type)   , intent(in)    :: temperature_inst
     type(soilstate_type)     , intent(in)    :: soilstate_inst
     type(soilhydrology_type) , intent(inout) :: soilhydrology_inst
     type(waterstatebulk_type)    , intent(inout) :: waterstatebulk_inst
     ...
     associate(                                                            & 
          ...
          watsat        => soilstate_inst%watsat_col
          qflx_lat_aqu  =>    waterstatebulk_inst%qflx_lat_aqu_col
          qflx_lat_aqu_layer  =>    waterstatebulk_inst%qflx_lat_aqu_layer_col
...
!----------------QCHARGE--------------------------------------------------
       ...
       do fc = 1, num_hydrologyc
          ...
          if (zwt(c) < frost_table(c) .and. t_soisno(c,k_frz) <= tfrz) then
             ...
             qflx_drain_perched(c) = 0._r8 !q_perch_max * q_perch &
                !  *(frost_table(c) - zwt(c))

             ! remove drainage from perched saturated layers
             rsub_top_tot = 0._r8 !-  qflx_drain_perched(c) * dtime
             do k = jwt(c)+1, k_frz
                rsub_top_layer= 0._r8 !max(rsub_top_tot,-(h2osoi_liq(c,k)-watmin))
                rsub_top_layer=0._r8 !min(rsub_top_layer,0._r8)
                rsub_top_tot = 0._r8 !rsub_top_tot - rsub_top_layer
             ...
             qflx_drain_perched(c) = 0._r8 !qflx_drain_perched(c) + rsub_top_tot/dtime
             ...
             if (use_vichydro) then
                ...
                rsub_top_max = 0._r8 !dsmax_tmp(c)
             else
                imped=10._r8**(-params_inst%e_ice*(icefracsum/dzsum))
                rsub_top_max = 0._r8 !10._r8 * sin((rpi/180.) * col%topo_slope(c))
             end if
#if (defined HUM_HOL)
         ...
         if (zwt(c) < 0.4_r8) then
           ...
           rsub_top(c)    = 0_r8
             ...
             if (use_vichydro) then
                ...
                rsub_top(c) = 0._r8 !imped * rsub_tmp
                ! make sure baseflow isn't negative
                rsub_top(c) = 0._r8 !max(0._r8, rsub_top(c))
             else
                rsub_top(c)    = 0._r8 !imped * rsub_top_max* exp(-fff(c)*zwt(c))
             end if
             ...
             if(jwt(c) == nlevsoi) then             
                ...
                rsub_top_tot = 0._r8 !- rsub_top(c) * dtime
                ...
                if(rsub_top_tot > 0.) then !rising water table
                   ...
                   if (use_vichydro) then
                      ...
                      do j = (nlvic(1)+nlvic(2)+1), nlevsoi
                         rsub_top_layer=0._r8 !max(rsub_top_tot, rsub_top_tot*hk_l(c,j)*dzmm(c,j)/wtsub_vic)
                         rsub_top_layer=0._r8 !min(rsub_top_layer,0._r8)
                      ...
                      do j = jwt(c)+1, nlevsoi
                         ...
                         rsub_top_layer=0._r8 !max(rsub_top_tot,-(s_y*(zi(c,j) - zwt(c))*1.e3))
                         rsub_top_layer=0._r8 !min(rsub_top_layer,0._r8)
                         h2osoi_liq(c,j) = h2osoi_liq(c,j) + rsub_top_layer
       ...
       do fc = 1, num_hydrologyc
          ...
          ! drainage
          rsub_top(c) = 0._r8 !rsub_top(c) - xs(c)/dtime
       ...
       do fc = 1, num_hydrologyc
          ...
          qflx_drain(c) = 0._r8 !qflx_rsub_sat(c) + rsub_top(c)
   ...
   subroutine PerchedLateralFlow(bounds, num_hydrologyc, &
     ...
     associate(                                                            & 
       ...
       do fc = 1, num_hydrologyc
          ...
          if (frost_table(c) > zwt_perched(c)) then
             ...
             if (col%is_hillslope_column(c) .and. col%active(c)) then
                ...
                qflx_drain_perched_vol(c) = 0._r8 !transmis*col%hill_width(c)*head_gradient
                qflx_drain_perched_out(c) = 0._r8 !1.e3_r8*(qflx_drain_perched_vol(c)/col%hill_area(c))
                ...
                qflx_drain_perched_out(c) = 0._r8 !q_perch_max * q_perch &
                  !   *(frost_table(c) - zwt_perched(c))
             endif
          endif
       ...
       do fc = 1, num_hydrologyc
          c = filter_hydrologyc(fc)
          ! drainage-out
          qflx_drain_perched(c) = 0._r8 !qflx_drain_perched(c) + qflx_drain_perched_out(c)
          if (col%is_hillslope_column(c) .and. col%active(c)) then
             ! drainage-in
             if (col%cold(c) /= ispval) then
                qflx_drain_perched(col%cold(c)) = 0._r8 !&
       ...
       do fc = 1, num_hydrologyc
          ...
          ! remove drainage from perched saturated layers
          drainage_tot = 0._r8 !qflx_drain_perched(c) * dtime
          ...
          qflx_drain_perched(c) = 0._r8 !qflx_drain_perched(c) - drainage_tot/dtime
       enddo
     ...
     associate(                                                            & 
      ...
      do fc = 1, num_hydrologyc
       ...
       do fc = 1, num_hydrologyc
          ...
          ! Sub-surface runoff and drainage
          qflx_drain(c) = 0._r8 !qflx_rsub_sat(c) + drainage(c)
